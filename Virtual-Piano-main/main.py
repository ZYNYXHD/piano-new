...
# Updated Playlist for each key sound including sharps
playlist = [
    './tones/C.wav',  # C
    './tones/C#.wav',  # C#
    './tones/D.wav',  # D
...
# Black keys are placed between white keys except between E-F and B-C
black_key_positions = [
    (i * 40 + 60, 0) if i % 7 != 2 and i % 7 != 6 else (-1, -1)  # Skip positions for E-F and B-C
    for i in range(14)
] + [
    (i * 40 + 80, 0) if i % 7 != 0 and i % 7 != 5 else (-1, -1)  # Add sharp positions
    for i in range(13)
]
...
def playMusic(p1, p2):
    """Plays music based on hand position on the virtual piano, including sharps."""
    ...
    for i, (bx, by) in enumerate(black_key_positions):
        if (bx < p1 < bx + w_black) and (by < p2 < by + h_black):  # Check if within bounds
            ...
            song = songs[i % len(songs)]  # Map to black keys
            song.play()
            time.sleep(0.1)  # Small delay to avoid multiple triggers
